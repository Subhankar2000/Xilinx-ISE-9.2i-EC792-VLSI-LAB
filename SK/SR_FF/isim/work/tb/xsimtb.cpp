...
{
    extern HSim__s6* createWork_sr_ff_behavioral(const char*);
    HSim__s6 *blk = createWork_sr_ff_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("sr_ff/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("sr_ff", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
