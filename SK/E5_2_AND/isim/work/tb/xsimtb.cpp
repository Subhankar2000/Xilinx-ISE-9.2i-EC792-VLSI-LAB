...
{
    extern HSim__s6* createWork_e5_2_and_behavioral(const char*);
    HSim__s6 *blk = createWork_e5_2_and_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("e5_2_and/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("e5_2_and", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
