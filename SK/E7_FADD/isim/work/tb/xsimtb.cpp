...
{
    extern HSim__s6* createWork_e7_fadd_behavioral(const char*);
    HSim__s6 *blk = createWork_e7_fadd_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("e7_fadd/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("e7_fadd", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
