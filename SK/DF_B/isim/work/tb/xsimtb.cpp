...
{
    extern HSim__s6* createWork_df_b_behavioral(const char*);
    HSim__s6 *blk = createWork_df_b_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("df_b/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("df_b", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
