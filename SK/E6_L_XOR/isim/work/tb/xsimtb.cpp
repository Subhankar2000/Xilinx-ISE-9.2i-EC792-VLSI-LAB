...
{
    extern HSim__s6* createWork_e6_l_xor_behavioral(const char*);
    HSim__s6 *blk = createWork_e6_l_xor_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("e6_l_xor/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("e6_l_xor", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
