...
{
    extern HSim__s6* createWork_xnorg_behavioral(const char*);
    HSim__s6 *blk = createWork_xnorg_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("xnorg/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("xnorg", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
