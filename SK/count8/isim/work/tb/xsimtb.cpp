...
{
    extern HSim__s6* createWork_count8_behavioral(const char*);
    HSim__s6 *blk = createWork_count8_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("count8/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("count8", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
