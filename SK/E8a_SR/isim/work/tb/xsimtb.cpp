...
{
    extern HSim__s6* createWork_e8a_sr_behavioral(const char*);
    HSim__s6 *blk = createWork_e8a_sr_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("e8a_sr/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("e8a_sr", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtb(const char*);
